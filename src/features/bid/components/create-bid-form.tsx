...
// #region CreateBidLoader

type CreateBidLoaderProps = {
  contractAddress: string
}

export const CreateBidLoader = ({ contractAddress }: CreateBidLoaderProps) => {
  ...
  const { auction, preference } = data

  return (
    <CountdownProvider endDate={auction.endDate} startDate={auction.startDate}>
      <CreateBidForm auction={auction} preference={preference} />
    </CountdownProvider>
  )
}
...
const CreateBidForm = ({ auction, preference }: CreateBidFormProps) => {
  ...
  const defaultValues = useMemo(
    () => ({
      accuracy: auction.preference === 0 && preference !== Infinity ? preference.toString() : "",
      time: auction.preference === 1 && preference !== Infinity ? preference.toString() : "",
      price:
        auction.preference === 2 && preference !== Infinity
          ? preference
